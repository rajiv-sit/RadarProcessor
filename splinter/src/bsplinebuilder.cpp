...
DenseVector BSpline::Builder::computeCoefficients(const BSpline& bspline) const
{
    ...
    {
        /*
         * The P-Spline is a smooting B-spline which relaxes the interpolation constraints on the control points to
         * allow smoother spline curves. It minimizes an objective which penalizes both deviation from sample points (to
         * lower bias) and the magnitude of second derivatives (to lower variance).
         *
...
SparseMatrix BSpline::Builder::getSecondOrderFiniteDifferenceMatrix(const BSpline& bspline) const
{
    ...
    for (unsigned int i = 0; i < numVariables; ++i)
        if (numBasisFunctions.at(i) < 3)
            throw Exception("BSpline::Builder::getSecondOrderDifferenceMatrix: Need at least three coefficients/basis "
                            "function per variable.");
...
std::vector<double> BSpline::Builder::knotVectorBuckets(const std::vector<double>& values,
                                                        unsigned int               degree,
